...
class RatInASphere(AgentCore):
    ...
    def obs_to_state(self, pos: np.ndarray):
        ...
        # Linearisation of indices
        phi, theta = pos
        dphi, dtheta = np.pi/(2*(self.n_stacks-1)), 2*np.pi/(self.n_slices-1)
        theta_index = round(theta/dtheta)
        phi_index = round((phi - np.pi/2)/dphi)
        curr_state = self.n_slices*phi_index + theta_index
    ...
    def create_transmat(self, plotting_variable: bool = False):
        """
        Creates the normalised transition matrix for a rectangular environment '2D_env'
        ...
        node_layout = np.arange(self.n_state).reshape(self.n_stacks, self.n_slices)
        self.xy = []
        ...
        for i in range(self.n_stacks):
            for j in range(self.n_slices):
                ...
                if i + 1 < self.n_stacks:
                    neighbours.append(node_layout[i + 1, j])
                if j - 1 >= 0:
                    neighbours.append(node_layout[i, j - 1])
                if j + 1 < self.n_slices:
                    neighbours.append(node_layout[i, j + 1])
